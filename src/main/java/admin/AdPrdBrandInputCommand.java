...
public class AdPrdBrandInputCommand implements AdminInterface {

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String realPath = request.getServletContext().getRealPath("/data/brand/");
	...
	// 파일 업로드 처리
			...
			// 전송폼의 값들 모두 받아오기
			String brandName = multipartRequest.getParameter("brandName")==null ? "" : multipartRequest.getParameter("brandName");
			...
			vo.setBrandFName(originalFileName);
			vo.setBrandFSName(filesystemname);
			AdDAO dao = new AdDAO();
			
			int res = dao.setBrandInput(vo);
			if(res == 1) {
